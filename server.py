...
def handle_connection(conn, addr, connection_id):
    ...
    while cnt < LIMIT:
        ...
        if data:
            ...
            if players_dict[connection_id].is_dead == False and players_dict[connection_id].hp <= 0:
                players_dict[connection_id].dead_time = time.time()
                players_dict[connection_id].is_dead = True

            # respawn after 3 seconds
            if players_dict[connection_id].is_dead and (time.time() - players_dict[connection_id].dead_time >= players_dict[connection_id].respawn_cooldown):
                players_dict[connection_id].is_dead = False
                players_dict[connection_id].is_respawn = True
                players_dict[connection_id].hp = players_dict[connection_id].max_hp
            ...
            # reset flag
            players_dict[connection_id].is_respawn = False
